...
<div id="wc-od">
	...
	<?php if ( 'calendar' === $delivery_option ) : ?>
		...
		<p><?php _e( 'We will contact you via email to arrange a suitable time for delivery on the desired day.', 'woocommerce-order-delivery' ); ?></p>
		<?php echo $delivery_date_field; ?>

	<?php elseif ( isset( $shipping_date ) ) : ?>

		<p><?php printf( __( 'Your order will be delivered on %s. We will contact you via email to arrange a suitable time for delivery.', 'woocommerce-order-delivery' ), "<strong>{$shipping_date}</strong>" ); ?></p>
		<p><?php printf(
			_n(
